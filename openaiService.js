...
async function callTool(name,args,ctx){
  ...
  const unauthorized = () => (ctx.transport === 'whatsapp' && !ctx.isOwnerMessage);
  async function runOwnerAction(actionName, target, fn) {
    if (unauthorized()) {
      return { error: 'Unauthorized: this is an owner-only action. Message it from your own Note-to-Self chat to authorize it.' };
    }
