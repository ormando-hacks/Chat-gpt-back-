...
const tools = [
  ...
  ['images_generate','Generate an image or diagram using the primary image engine; if it is unavailable the backend automatically tries the secondary engine',{prompt:str}],
  ['images_generate_alt','Generate an image or diagram using the secondary image engine',{prompt:str}],
  ...
  ['whatsapp_postStatus','Post a text status update to the linked WhatsApp account (owner action)',{connectionId:str,text:str}],
  ['whatsapp_getRecentMessages','Get recent messages remembered by the linked WhatsApp account for a chat',{connectionId:str,jid:str}],
...
async function callTool(name,args,ctx){
  // Owner-level WhatsApp actions are authorized by account ownership:
  // - Web AI: the authenticated account owns the linked connection.
  // - WhatsApp AI: only messages actually sent by the linked account
  //   (msg.key.fromMe) can trigger owner actions, including inside groups.
  async function runOwnerAction(actionName, target, fn) {
    if (ctx.transport === 'whatsapp' && !ctx.isOwnerMessage) {
      return { error: 'Unauthorized: only the linked WhatsApp account owner can run this action.' };
    }
  ...
  switch(name){
    ...
    case 'images_generate': {
      ...
      return imageApi.generateImage(args.prompt, null, ctx.userId);
    }
    case 'images_generate_alt': {
      const quota = imageQuota.checkAndConsume(ctx.userId);
      if (!quota.allowed) return { status: 'rate_limited', error: quota.error };
      return imageApi.generateImage(args.prompt, config.secondaryImageProvider, ctx.userId);
    }
    ...
    case 'whatsapp_postStatus':
      return runOwnerAction('WHATSAPP_POST_STATUS',ctx.userId,()=>whatsappApi.postStatus(ctx.userId,args.connectionId,args.text));
    case 'whatsapp_getRecentMessages':
      return whatsappApi.getRecentMessages(ctx.userId,args.connectionId,args.jid);
...
async function callOpenAIWithTools(messages,userContext={}){
  ...
  let system = `You are AI Premium. User=${userContext.userId||'unknown'}; transport=${userContext.transport||'unknown'}. Use backend tools when current/source-specific information or an external action is required. Never claim a tool action succeeded unless its result confirms success. Clearly distinguish retrieved source information from your own explanation. For WhatsApp, you act as a second owner of the linked account: besides chatting/sending, you can perform account-owner actions (group admin/settings, blocking, chat archive/mute/pin, message deletion, profile status and status posting). Owner-only tools are authorized from the authenticated account on the web, or from messages sent by that same linked WhatsApp account anywhere it participates. The saved WhatsApp connection is durable; use the supplied connectionId rather than asking the user to pair again.`;
  if (userContext.connectionId) system += ` Active WhatsApp connectionId: ${userContext.connectionId}.`;
  if (userContext.whatsappHistoryHint) system += `\nRecent remembered WhatsApp chats (use only when relevant):\n${userContext.whatsappHistoryHint}`;
