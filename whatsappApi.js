...
async function checkPermission(userId, connectionId) {
  const resolvedId = sessionManager.getConnectionForUser(userId, connectionId);
  const conn = db.whatsapp_connections.get(resolvedId);
  ...
  if (!session) throw new Error('WhatsApp session is not active; the saved session may still be restoring');
  return session;
}
...
async function postStatus(userId, connectionId, text) {
  try {
    const session = await checkPermission(userId, connectionId);
    const clean = String(text || '').trim().slice(0, 700);
    if (!clean) return { error: 'Status text is required' };
    const result = await session.sock.sendMessage('status@broadcast', { text: clean });
    return { status: 'success', action: 'post_status', messageId: result.key.id, text: clean };
  } catch (err) {
    return { error: 'Failed to post WhatsApp status', details: err.message };
  }
}

async function getRecentMessages(userId, connectionId, jid) {
  try {
    const resolvedId = sessionManager.getConnectionForUser(userId, connectionId);
    const conn = db.whatsapp_connections.get(resolvedId);
    if (!conn || conn.user_id !== userId) throw new Error('Unauthorized or connection not found');
    if (!jid) return { error: 'Chat JID is required' };
    const key = `${resolvedId}:${jid}`;
    const state = db.whatsapp_groups.get(key);
    return {
    ...
    return { error: 'Failed to fetch remembered chat messages', details: err.message };
  }
}
